(function startYouTubeTranscriptExtension() {
  ...
  function updatePanelContent(videoId, shouldResetTranscriptState) {
    var videoIdElement = document.querySelector("[data-yt-transcript-video-id]");
    var titleElement = document.querySelector("[data-yt-transcript-video-title]");
    var statusLabelElement = document.querySelector("[data-yt-transcript-status-label]");
    var statusMessageElement = document.querySelector("[data-yt-transcript-status-message]");
    var transcriptPlaceholderElement = document.querySelector("[data-yt-transcript-placeholder]");
    var transcriptStateElement = document.querySelector("[data-yt-transcript-state]");
    ...
    var shell = sidebar && typeof sidebar.createSidebarShellState === "function"
  ...
  function clearLanguageOptions() {
    var languageWrap = document.querySelector("[data-yt-transcript-language-wrap]");
    var languageSelect = document.querySelector("[data-yt-transcript-language-select]");
  ...
  function setLanguageOptions(languages) {
    ...
    languages.forEach(function addLanguageOption(language, index) {
      ...
      option.textContent = language.label + " - " + kindLabel;
  ...
  function setTranscriptControlsEnabled(isEnabled) {
    ...
    var actionButtons = document.querySelectorAll("[data-yt-transcript-action]");
  ...
  function renderTranscriptRows(rows) {
    ...
    var visibleRows = Array.isArray(rows) ? rows.filter(function filterRow(row) {
      return !query || row.text.toLowerCase().indexOf(query) !== -1 || row.timestamp.indexOf(query) !== -1;
    }) : [];
    ...
    if (visibleRows.length === 0) {
      if (transcriptStateElement) {
        transcriptStateElement.textContent = rows.length > 0 ? "No matches" : "Empty";
      }

      if (transcriptPlaceholderElement) {
        transcriptPlaceholderElement.hidden = false;
        transcriptPlaceholderElement.textContent = rows.length > 0 ? "No transcript rows match your search." : "No readable transcript rows were found.";
      }
    ...
    visibleRows.forEach(function addTranscriptRow(row) {
      ...
      var timestampButton = createElement("button", "yt-transcript-helper-row-time", row.timestamp);
      var textElement = createElement("span", "yt-transcript-helper-row-text", row.text);

      timestampButton.type = "button";
      timestampButton.setAttribute("data-yt-transcript-start", String(row.startSeconds));
    ...
    if (transcriptStateElement) {
      transcriptStateElement.textContent = query ? visibleRows.length + " matches" : rows.length + " rows";
    }
  ...
  function getSelectedTrack() {
    ...
    return currentCaptionTracks[selectedIndex] || currentCaptionTracks[0] || null;
  }
  ...
  function getTranscriptDebugOptions() {
    ...
    try {
      isEnabled = window.localStorage && window.localStorage.getItem("ytTranscriptHelperDebug") === "true";
    } catch (error) {
      isEnabled = false;
    }

    if (!isEnabled || !window.console || typeof window.console.debug !== "function") {
      return undefined;
    }
  ...
  function fetchAndRenderSelectedTranscript() {
    ...
    if (!selectedTrack) {
      showTranscriptMessage("No transcript available for this video", "No captions");
      return;
    }

    logTranscriptDebug("Selected video ID: " + (currentVideoId || "unknown"));
    logTranscriptDebug("Caption track source: " + (currentCaptionSource || "unknown"));
    logTranscriptDebug(
      "Selected caption track: " +
      "label=\"" + (selectedTrack.label || "unknown") + "\", " +
      "language=\"" + (selectedTrack.languageCode || "unknown") + "\", " +
      "kind=\"" + (selectedTrack.kind || "unknown") + "\""
    );

    showTranscriptMessage("Loading transcript rows...", "Loading");

    if (!transcript || typeof transcript.fetchTranscriptRowsWithFallbacks !== "function") {
      showTranscriptMessage("Transcript loading is not available in this build.", "Error");
      return;
    }
    ...
    transcript.fetchTranscriptRowsWithFallbacks({
      ...
      debugLogger: getTranscriptDebugOptions() && getTranscriptDebugOptions().debugLogger
      ...
      if (!result || !result.ok) {
        currentTranscriptRows = [];
        setTranscriptControlsEnabled(false);
        showTranscriptMessage("No YouTube transcript is available for this video.\nAudio transcription can be added later.", "No transcript");
        return;
      }
      ...
      logTranscriptDebug("Transcript pipeline failed: " + (error && error.message ? error.message : "Unknown error"));
      currentTranscriptRows = [];
      setTranscriptControlsEnabled(false);
      showTranscriptMessage("No YouTube transcript is available for this video.\nAudio transcription can be added later.", "No transcript");
    });
  ...
  function getNativeTranscriptRows(transcript) {
    if (!transcript || typeof transcript.parseNativeTranscriptRowsFromDocument !== "function") {
      return [];
    }
    ...
    try {
      ...
      logTranscriptDebug("Native transcript parser failed: " + (error && error.message ? error.message : "Unknown error"));
      return [];
    }
  }
  ...
  function findNativeTranscriptButton(label) {
    ...
    return buttons.find(function findButton(button) {
  ...
  function waitForNativeTranscriptRows(transcript) {
    ...
    return new Promise(function waitForRows(resolve) {
      function checkRows() {
        ...
        if (rows.length > 0 || Date.now() - startTime >= NATIVE_TRANSCRIPT_FALLBACK_TIMEOUT_MS) {
          resolve(rows);
          return;
        }
  ...
  async function fetchNativeTranscriptRows(transcript) {
    ...
    if (rows.length > 0) {
      logTranscriptDebug("Native transcript panel already has " + rows.length + " rows");
      return rows;
    }
    ...
    if (!showButton) {
      logTranscriptDebug("Native transcript fallback skipped: Show transcript button was not found");
      return [];
    }

    logTranscriptDebug("Timedtext was empty; opening YouTube native transcript panel fallback");
    showButton.click();
  ...
  async function copyTranscriptToClipboard() {
    ...
    var helpers = getYouTubeHelpers();
    var videoTitle = helpers ? helpers.getCurrentVideoTitle(document) : "YouTube transcript";
    var text;

    if (!exportHelpers || typeof exportHelpers.buildPlainTextTranscript !== "function" || currentTranscriptRows.length === 0) {
      return;
    }

    text = exportHelpers.buildPlainTextTranscript(videoTitle, window.location.href, currentTranscriptRows);
  ...
  function exportTranscript(format) {
    ...
    if (isMarkdown) {
      text = exportHelpers.buildMarkdownTranscript(videoTitle, window.location.href, currentTranscriptRows);
  ...
  function handleTranscriptAction(event) {
    var action = event.target && event.target.getAttribute("data-yt-transcript-action");
    ...
    if (action === "copy") {
      copyTranscriptToClipboard().catch(function handleCopyError() {
        showTranscriptMessage("Copy failed. Your browser may have blocked clipboard access.", "Error");
      });
    } else if (action === "txt") {
  ...
  function applyTranscriptAvailabilityResult(result) {
    ...
    if (status === "checking") {
      ...
      currentCaptionTracks = Array.isArray(safeResult.tracks) ? safeResult.tracks : [];
      currentTranscriptRows = [];
      currentSearchQuery = "";
      ...
      placeholderText = "No YouTube transcript is available for this video. Audio transcription can be added later.";
      clearLanguageOptions();
      clearTranscriptRows();
      setTranscriptControlsEnabled(false);
    } else {
  ...
  function startTranscriptAvailabilityCheck(videoId) {
    ...
    if (transcript && typeof transcript.createCheckingAvailabilityResult === "function") {
      applyTranscriptAvailabilityResult(transcript.createCheckingAvailabilityResult());
    } else {
    ...
    if (!transcript || typeof transcript.detectTranscriptAvailability !== "function") {
      applyTranscriptAvailabilityResult({
        status: "failed",
  ...
  function createToggleButton() {
    ...
    toggle = createElement("button", "yt-transcript-helper-toggle", "Transcript");
    toggle.id = TOGGLE_ID;
    toggle.type = "button";
    toggle.hidden = true;
  ...
  function createPanel(videoId) {
    ...
    var heading = createElement("h2", "yt-transcript-helper-heading", shell.extensionName);
    var subtitle = createElement("p", "yt-transcript-helper-subtitle", "Transcript sidebar");
    var closeButton = createElement("button", "yt-transcript-helper-icon-button", "X");
    ...
    var statusHeader = createElement("div", "yt-transcript-helper-status-header");
    var statusLabel = createElement("span", "yt-transcript-helper-status-label", shell.statusLabel);
    var statusMessage = createElement("span", "yt-transcript-helper-status-message", shell.statusMessage);
    var videoIdRow = createElement("div", "yt-transcript-helper-video-id-row");
    var videoIdLabel = createElement("span", "yt-transcript-helper-video-id-label", "Video ID");
    var statusValue = createElement("code", "yt-transcript-helper-video-id", shell.videoId);
    var languageWrap = createElement("label", "yt-transcript-helper-language");
    var languageLabel = createElement("span", "yt-transcript-helper-language-label", "Language");
    var languageSelect = createElement("select", "yt-transcript-helper-language-select");
    ...
    var videoTitleElement = createElement("p", "yt-transcript-helper-video-title", shell.videoTitle);
    ...
    shell.actions.forEach(function addActionButton(action) {
      var button = createElement("button", "yt-transcript-helper-secondary-button", action.label);
      button.type = "button";
      button.disabled = action.disabled;
      button.setAttribute("data-yt-transcript-action", action.label.toLowerCase());
    ...
    var transcript = createElement("section", "yt-transcript-helper-transcript");
    var transcriptHeader = createElement("div", "yt-transcript-helper-transcript-header");
    var transcriptTitle = createElement("h3", "yt-transcript-helper-transcript-title", shell.transcriptTitle);
    var transcriptState = createElement("span", "yt-transcript-helper-transcript-state", "Not loaded");
    var transcriptPlaceholder = createElement("p", "yt-transcript-helper-transcript-placeholder", shell.transcriptPlaceholder);
    var transcriptRows = createElement("div", "yt-transcript-helper-rows");

    transcriptState.setAttribute("data-yt-transcript-state", "true");
    transcriptPlaceholder.setAttribute("data-yt-transcript-placeholder", "true");
  ...
  function renderForCurrentPage() {
    ...
    if (navigation && typeof navigation.getPageAction === "function") {
      action = navigation.getPageAction(currentVideoId, window.location.href, helpers.getVideoIdFromUrl);
    } else {
      var fallbackVideoId = helpers.getCurrentVideoId();
      action = fallbackVideoId ? { type: "render", videoId: fallbackVideoId } : { type: "remove", videoId: null };
    }
  ...
  function listenForYouTubeNavigation() {
    ...
    document.addEventListener("yt-navigate-finish", handleYouTubeNavigationFinish);
    window.addEventListener("yt-transcript-helper-location-change", handlePossibleNavigation);
